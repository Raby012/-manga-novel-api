 ...
 * ─────────────────────────────────────────────────────────────────────────────
 * Custom scraper for WeebCentral (weebcentral.com).
 *
 * WeebCentral is a popular manhwa-focused reader. It does not have a public
 * API, so we scrape the HTML directly. The site is behind Cloudflare, so we
 * use fetchHTML() (which uses got-scraping with Chrome fingerprinting).
 *
 * CONTENT TYPES
 * ──────────────
 * WeebCentral hosts manga, manhwa, and manhua. The series page includes a
 * "Type" field that we extract to set the `contentType` field.
 */
...
export interface WCResult {
  id: string;           // path slug, e.g. "series/0101be22/..."
  title: string;
...
export class WeebCentralScraper {
  ...
  async search(query: string, page = 1): Promise<WCResult[]> {
    ...
    $("article.series-item, .manga-item, [class*='series']").each((_i, el) => {
      ...
      const cover   = anchor.find("img").attr("src") ?? anchor.find("img").attr("data-src") ?? "";
  ...
  async fetchSeriesInfo(id: string): Promise<{
    title: string;
    contentType: WCResult["contentType"];
    coverUrl: string;
    status: string;
    genres: string[];
    description: string;
    ...
    const html = await fetchHTML(`${BASE}/${id}`, { headers: { Referer: BASE } });
    const $ = cheerio.load(html);

    const title   = $("h1").first().text().trim();
    const cover   = $(".series-image img, .cover img").attr("src") ?? "";
    const desc    = $(".summary, .synopsis, [class*='description']").first().text().trim();
    const genres:  string[] = [];
    ...
    $(".info-item, .series-info li").each((_i, el) => {
      ...
      if (label.includes("genre"))   genres.push(...value.split(",").map(s => s.trim()));
  ...
  async fetchChapterPages(chapterId: string): Promise<string[]> {
    const html = await fetchHTML(`${BASE}/${chapterId}`, {
