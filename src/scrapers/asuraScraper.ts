 ...
 * asuraScraper.ts
 * ─────────────────────────────────────────────────────────────────────────────
 * Scraper for AsuraScans (asuracomic.net) — one of the most popular
 * manhwa / manga scanlation sites.
 *
 * Behind Cloudflare → we use fetchHTML (got-scraping with Chrome fingerprint).
 * AsuraScans has no public API, so all data is extracted from HTML via cheerio.
 */
...
export class AsuraScraper {
  ...
  async search(query: string): Promise<AsuraResult[]> {
    const html = await fetchHTML(
      `${BASE}/series?name=${encodeURIComponent(query)}`,
      { headers: { Referer: BASE } }
    );
    const $ = cheerio.load(html);
    const results: AsuraResult[] = [];

    $("div.grid > a, .series-card, [class*='SeriesCard']").each((_i, el) => {
      ...
      const rating  = $(el).find("[class*='rating'], [class*='Rating']").text().trim();
      const typeEl  = $(el).find("[class*='type'], [class*='Type']").text().trim();

      if (slug && title) {
        results.push({
          slug,
          title,
          contentType: parseType(typeEl),
          coverUrl:    cover,
          status,
  ...
  async fetchSeriesInfo(slug: string): Promise<{
    title: string;
    contentType: AsuraResult["contentType"];
    coverUrl: string;
    status: string;
    genres: string[];
    description: string;
    ...
    const html = await fetchHTML(`${BASE}/${slug}`, { headers: { Referer: BASE } });
    const $ = cheerio.load(html);

    const title   = $("h1, .series-title").first().text().trim();
    const cover   = $(".series-thumb img, .thumb img").attr("src") ?? "";
    const desc    = $(".series-desc, .description, [class*='Synopsis']").text().trim();
    const genres:  string[] = [];
  ...
  async fetchChapterPages(chapterSlug: string): Promise<string[]> {
    const html = await fetchHTML(`${BASE}/${chapterSlug}`, {
      headers: { Referer: BASE },
    });
    const $ = cheerio.load(html);
    const pages: string[] = [];
